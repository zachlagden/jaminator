...
- [ ] **WIFI-03**: Wi-Fi profile passwords are delivered to fleet laptops via a **private GitHub manifest repo** gated by a fine-grained read-only Personal Access Token bundled in the MSI. Public manifest (still in the public `zachlagden/jaminator` repo) carries only non-sensitive config. Private manifest repo carries credentials. PAT rotation cycle aligns with PSK rotation cycle (e.g., termly). This is "operational security" — not cryptographic perfection (the PAT in the MSI is extractable by RE) — but it removes PSKs from the search-engine-indexable internet, which is the meaningful threat for a school Wi-Fi network, and it requires **zero per-laptop touch** (the entire reason Jaminator exists: Jam Coding staff are not technicians). Encryption-at-rest in manifest was considered and rejected because every variant ends in rot13 once the bundled MSI is public. See `## Key Decisions` for the full rationale.
...
| **WIFI-03: private manifest repo + PAT-in-MSI (not encryption-at-rest)** | Hard product constraint from Jam Coding: staff are not technicians; per-laptop secret distribution is a non-starter. Every "encrypt PSK in public manifest with key in public MSI" scheme is rot13 (the key is on the public MSI). The only design that keeps PSKs off the public search-indexable internet **without per-laptop touch** is moving sensitive manifest content into a private GitHub repo and bundling a read-only PAT in the MSI. Threat-model honest: an attacker willing to RE the public MSI can still extract the PAT (mitigated by termly rotation aligned with PSK rotation), and any admin on a deployed laptop can still run `netsh wlan show profile key=clear` (unsolvable on Windows; same exposure as any other PSK distribution method including Intune). Encryption-at-rest in the manifest is explicitly rejected because it implies a stronger security guarantee than it delivers. | — Pending (Milestone 2) |
